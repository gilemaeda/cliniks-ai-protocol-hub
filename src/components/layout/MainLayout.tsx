...
export default function MainLayout({ children }: MainLayoutProps) {
  ...
  useEffect(() => {
    const fetchClinic = async () => {
      if (!user) return;
      const { data, error } = await supabase.rpc('get_user_clinic_data', { user_uuid: user.id });
      if (!error && data && data.length > 0) {
        const { clinic_id } = data[0];
        if (clinic_id) {
    ...
    <div className="flex flex-row justify-between items-center py-2 gap-2 min-h-[56px]">
      ...
      <div className="flex items-center gap-2">
        <img
          ...
          alt="Cliniks IA Logo"
          className="h-10 w-10 rounded-full bg-white shadow-md object-contain border-2 border-pink-400"
        />
        <div>
      ...
      {/* Banner global */}
      {clinic?.banner_url && (
        <div style={{ width: '100%', height: '140px', backgroundColor: '#2d133b', overflow: 'hidden', marginBottom: 0, borderBottomLeftRadius: 12, borderBottomRightRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <img
            src={clinic.banner_url && clinic.banner_url.startsWith('http') ? clinic.banner_url : `https://rpfrmclsraiidjlfeonj.supabase.co/storage/v1/object/public/clinic-assets/${clinic.banner_url}`}
